...
Jarvis is a browser-based, voice-driven personal assistant with a sci-fi / Jarvis-style interface. It runs as a local Next.js app and uses locally hosted Ollama models for the AI conversation and tool-calling loop.

The intended flow is simple: speak or type to Jarvis, let it decide whether it needs a tool, show useful results in the HUD, then have the answer spoken back.
...
- **Three.js**, `@react-three/fiber`, `@react-three/drei`, and `@react-three/postprocessing` for the animated 3D Jarvis/HUD scene
...
| --- | --- | --- |
| Main assistant / tool calling | `qwen3:8b` | The normal Jarvis “brain.” |
...
- Show an animated 3D Jarvis visual and HUD-style interface.
- Chat through a local Ollama model.
- Search the web through Tavily when `TAVILY_API_KEY` is configured.
- Read and list files inside a restricted `JARVIS_FILES_DIR` directory.
...
- A spoken “processing” or “searching” status line plays while Jarvis waits for an answer.
- Microphone recognition is paused during speech so Jarvis does not hear its own voice, then resumes after playback.
...
TAVILY_API_KEY=...
JARVIS_FILES_DIR=/absolute/path/to/jarvis-files
```
...
components/
  ...
  jarvis/                   Three.js animated Jarvis scene
lib/
